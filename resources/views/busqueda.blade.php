    ...
    <form id="searchForm" class="flex flex-col gap-4 w-full max-w-md">
        <div class="relative">
            <input
                ...
                required
                autocomplete="off"
                class="rounded-lg border border-gray-300 px-4 py-2 focus:ring-2 focus:ring-indigo-400 focus:outline-none transition w-full"
            />
            <ul
                id="autocomplete-list"
                class="absolute z-10 left-0 right-0 bg-white border border-gray-200 rounded-b-lg shadow-lg max-h-48 overflow-y-auto hidden"
            ></ul>
        </div>

        <button
            type="submit"
            class="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 rounded-lg shadow transition active:scale-95"
        >
            Buscar
        </button>
    </form>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function mostrarSpinner() {
        resultadosDiv.innerHTML = `
            <div class='flex justify-center'>
                <div class='animate-spin rounded-full h-10 w-10 border-t-4 border-indigo-600 border-solid'></div>
            </div>
            <p class='text-center text-indigo-500 mt-2'>Buscando...</p>
        `;
    }

    function mostrarError(mensaje) {
        resultadosDiv.innerHTML = `
            <div class='bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg text-center animate-fade-in'>
                ${mensaje}
            </div>
        `;
    }
    ...
    function mostrarVacio() {
        resultadosDiv.innerHTML = `
            <div class='bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded-lg text-center animate-fade-in'>
